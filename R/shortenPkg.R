...
shortenPkg <- function(pkg, addPrefix = T, silent = F,num.prefix=3) {
  ...
  if (length(grep(sprintf("package:%s", pkg), search()))) {
    ...
    prepDTforRef00 = function(){return(options()$.funCNamesPkgReference);}
    #detach if it already exists
    discardShortcodes(pkg=pkg,reflib=prepDTforRef00(),response=F)
    ...
    for (i in functionlist) {
      ...
      if(!is.null(newShortName)){
        ...
        try(assign(newShortName, eval(parse(text = as.character(i))), envir = .spkg), silent = T)
      }
      else{
        if(!silent)cat(paste0('short name found for ',pkg,'::',i,'\n'))
      }
    ...
    if(!silent) cat(paste0('Successfully shortened ',pkg,'. To view the functions and their equivalence, enter index("',pkg,'")\n'))
  }
  else {
